...
def optimizeAttack(attackable, multiAttackable, goldOrEnergyChoice, enemyBaseChoice):    #detect aggressive AI                                                       
    ...
    if g.energy >= 85:
            boost = True
    if g.energyCellNum >= 4 and g.goldCellNum >= 3:
        for i in range(len(enemyBaseChoice)):
            if not enemyBaseChoice[i].isBuilding and g.energy >= 40:
                if len(adjacentCells(enemyBaseChoice[i], 'others')) == len(adjacentCells(enemyBaseChoice[i])) - 2:
                    if len(adjacentCells(enemyBaseChoice[i])) == 4 and isColinear(adjacentCells(enemyBaseChoice[i], 'others')[0], adjacentCells(enemyBaseChoice[i], 'others')[1]) or len(adjacentCells(enemyBaseChoice[i])) == 3 and isColinear(adjacentCells(enemyBaseChoice[i], 'others')[0], adjacentCells(enemyBaseChoice[i], 'none')[0]):
                        dx1 = enemyBaseChoice[i].x - adjacentCells(enemyBaseChoice[i], 'self')[0].x 
                        dy1 = enemyBaseChoice[i].y - adjacentCells(enemyBaseChoice[i], 'self')[0].y 
                        dx2 = enemyBaseChoice[i].x - adjacentCells(enemyBaseChoice[i], 'self')[1].x 
                        dy2 = enemyBaseChoice[i].y - adjacentCells(enemyBaseChoice[i], 'self')[1].y
                        for j in range(1, 4):
                            c1 = g.GetCell(enemyBaseChoice[i].x + j*dx1, enemyBaseChoice[i].y + j*dy1)
                            c2 = g.GetCell(enemyBaseChoice[i].x + j*dx2, enemyBaseChoice[i].y + j*dy2)
                ...
                if len(adjacentCells(enemyBaseChoice[i], 'others')) == len(adjacentCells(enemyBaseChoice[i])) - 1:
                    if len(adjacentCells(enemyBaseChoice[i])) == 2: 
                        if isColinear(adjacentCells(enemyBaseChoice[i], 'others')[0], adjacentCells(enemyBaseChoice[i], 'none')[0]):
                            dx = enemyBaseChoice[i].x - adjacentCells(enemyBaseChoice[i], 'none')[1].x 
                            dy = enemyBaseChoice[i].y - adjacentCells(enemyBaseChoice[i], 'none')[1].y 
                        if isColinear(adjacentCells(enemyBaseChoice[i], 'others')[0], adjacentCells(enemyBaseChoice[i], 'none')[1]):
                            dx = enemyBaseChoice[i].x - adjacentCells(enemyBaseChoice[i], 'none')[0].x 
                            dy = enemyBaseChoice[i].y - adjacentCells(enemyBaseChoice[i], 'none')[0].y 
                    else:
                        dx = enemyBaseChoice[i].x - adjacentCells(enemyBaseChoice[i], 'self')[0].x 
                        dy = enemyBaseChoice[i].y - adjacentCells(enemyBaseChoice[i], 'self')[0].y 
                    for j in range(1, 4):
                        c = g.GetCell(enemyBaseChoice[i].x + j*dx, enemyBaseChoice[i].y + j*dy)
    ...
    if multiAttackable and g.gold >= (3 - g.baseNum) * 60 + 40:    
        openCells = [c for c in attackable if takeTime(c) <= 5]
        for i in range(len(multiAttackable)):
            ...
            for t in multiTargets:
                if g.energyCellNum >= 4 and g.goldCellNum >= 3:
                    if openCells and t.owner != g.uid:
                        for j in range(len(enemyBaseChoice)):
                            if t == min(openCells, key=lambda c: distance(c, enemyBaseChoice[j])) and distance(t, enemyBaseChoice[j]) >= 1:
                                if j == 0:
                                    values1[i] += 2.5 
                                elif j == 1:
                                    values1[i] += 1.5
                ...
                if t.isBase:
                    ...
                    if openCells and t.owner != g.uid:
                        for j in range(len(goldOrEnergyChoice)):
                            if t == min(openCells, key=lambda c: distance(c, goldOrEnergyChoice[j])):
                                if j == 0:
                                    values1[i] += 1.25
                                elif j == 1:
                                    values1[i] += 0.75
                    ...
                    if isAroundMyCell(t, 'gold'):   
                        values1[i] += 1.5 if t.owner != g.uid else 0.25
                    if isAroundMyCell(t, 'energy'):
                        values1[i] += 1.5 if t.owner != g.uid else 0.25
                    if isAroundMyCell(t, 'base'):
                        values1[i] += 3 if t.owner != g.uid else 0.5
                    if isAroundMyCell(t, 'gold', 2):
                        values1[i] += 0.6 if t.owner != g.uid else 0.1
                    if isAroundMyCell(t, 'energy', 2):
                        values1[i] += 0.6 if t.owner != g.uid else 0.1
                    if isAroundMyCell(t, 'base', 2):
                        values1[i] += 0.75 if t.owner != g.uid else 0.125
                    if t.owner == g.users[0].id:
                        values1[i] += 1
                    if len(g.users) > 1 and g.users[0].id == g.uid and (isOwnedByCompetitors(t, 50) or t.owner == g.users[1].id):
                        values1[i] += 1
    openCells = [c for c in attackable if takeTime(c, g.energy >= 45 + g.energy*0.05) <= 5]
    for i in range(len(attackable)):
        values2 += [33 - takeTime(attackable[i], boost)]
        if g.energyCellNum >= 4 and g.goldCellNum >= 3:
            if openCells:
                for j in range(len(enemyBaseChoice)):
                    if attackable[i] == min(openCells, key=lambda c: distance(c, enemyBaseChoice[j])) and distance(attackable[i], enemyBaseChoice[j]) >= 1:
                        if j == 0: 
        ...
        if not attackable[i].isBase:
            if openCells:
                for j in range(len(goldOrEnergyChoice)):
                    if attackable[i] == min(openCells, key=lambda c: distance(c, goldOrEnergyChoice[j])):
                        if j == 0: 
                            if takeTime(attackable[i]) > 2.5 and g.energy >= 45 + g.energy*0.05 and not boosted[i]:
                                values2[i] += takeTime(attackable[i]) - takeTime(attackable[i], True)
                                boosted[i] = True
        ...
        if abs(energyGoldRatio - desiredRatio) > 0.5:
            ...
            for i in range(len(unclaimed)):
                if unclaimed[i].cellType == ('gold' if energyGoldRatio > desiredRatio else 'energy'):
                    ...
                    insert_ind += 1
                if insert_ind == (3 if abs(energyGoldRatio - desiredRatio) > 1 else 2):
                    break
    return unclaimed 
...
def nearbyEnemyBases(enemyBases, myBoundary):  
    global targetID
    baseTargets = []
    if enemyBases and g.energyCellNum >= 4 and g.goldCellNum >= 3:
    ...
    for gold in myGold:
        if gold.isTaking:
            ...
            if max(takeTime(c) for c in adjacentCells(gold)) < 2.5 and g.gold >= (3 - g.baseNum) * 60 + 40:
                ...
                for c in adjacentCells(gold, 'me'):                #multiAttack maybe
                    if takeTime(c) < 2 and isEnemyApproaching(c):
                        target[0] = c
                        break
        else:    
